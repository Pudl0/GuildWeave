...
GuildWeave.PopupEditMode = GuildWeave.PopupEditMode or {}
...
local function BuildOverlay(spec)
    ...
    f:SetBackdrop(GuildWeave.Constants.POPUPBACKDROP)
    ...
    f:SetScript("OnDragStop", function(self)
        ...
        GuildWeave:SaveFramePosition(self, spec.dbKey)
    end)

    GuildWeave:RestoreFramePosition(f, spec.dbKey, spec.point, spec.x, spec.y)
...
local function BuildContainer()
    ...
    title:SetText(Localization["POPUPEDIT_TITLE"])
    ...
    hint:SetText(Localization["POPUPEDIT_HINT"])
...
local function BuildControls()
    ...
    saveBtn:SetText(Localization["POPUPEDIT_SAVE_BTN"])
...
function EditMode:Enable()
    ...
    for i, frame in ipairs(overlays) do
        local spec = SPECS[i]
        if spec then
            GuildWeave:RestoreFramePosition(frame, spec.dbKey, spec.point, spec.x, spec.y)
        end
        frame:Show()
    end

    enabled = true
    GuildWeave:Print(Localization["POPUPEDIT_ENABLED"])
end
...
function EditMode:Disable()
    ...
    enabled = false
    GuildWeave:Print(Localization["POPUPEDIT_DISABLED"])
end
...
SLASH_GWPOPUPEDIT1 = "/gwedit"
SlashCmdList["GWPOPUPEDIT"] = function()
    EditMode:Toggle()
end
